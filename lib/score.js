...
const TITLES = ['Novice', 'Journeyman', 'Expert', 'Master', 'Grandmaster']; // four levels each
...
function summarize(names, known) {
...
function score(summary, known) {
  ...
  const level = Math.max(1, Math.min(20, Math.round(Object.values(parts).reduce((a, b) => a + b, 0))));
  ...
  const klass = total < 3 || best[1] === 0 ? 'Wanderer' : best[0];

  const rarest = products.slice().sort((a, b) => stats.get(a)[1] - stats.get(b)[1])[0] || null;
  return {
    level, title: TITLES[Math.ceil(level / 4) - 1], class: klass, total, count,
...
module.exports = { summarize, validSummary, score, suggest, KINDS, TITLES };
